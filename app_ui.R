...
intro <- tabPanel(
  ...
  tags$body(
    mainPanel(
      h4("Augene Pak, Brian Park, Jonathan Seto, Joseph Altamira"),
      h1("Introduction"),
      ...
      recognized that inequities due to socioeconomic status can reveal themselves in diseases as evident with COVID-19 on underrepresented
      ...
      p("The dataset is from the CDC and without any cleaning it is too large to upload GitHub. Hence, we filtered the data using the prep_data.R
      ...
      that did not have any data, which is an indicator of regulatory rules in that county or a reflection of their stance on aggregated
...
map <- tabPanel(
  ...
  sidebarLayout(
    ...
    mainPanel(
      plotlyOutput("map"),
      h1("Insights"),
      p("A major geographical trend is that Southern states such as Arkansas, Alabama, Mississippi, and Georgia tend to have lower income and higher
        mortality rates. For these states, the same trend is true for incidence rates. In contrast, states in the Pacific Northwest tend to have higher
        income but lower mortality."),

      p("Another noticeable trend is that high density populations areas and metropolitan cities have a higher income and higher incidence along with lower mortality
